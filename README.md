...
| **exclude_keys_in_path** | `[{"path":"full","keys":["info"]}]` | Исключать ключ только внутри пути: например убрать поле `info` только внутри раздела `full` (на остальных путях `info` остаётся). |
| **column_order** | `["empName","empFamilyName","jobTitle"]` | Порядок колонок на листе: сначала перечисленные, затем остальные в алфавитном порядке. |
...
**Массивы объектов в значениях:** если значение поля — массив объектов `[{key: value}, ...]`, оно разворачивается в колонки вида `имя_поля - ключ_объекта`; значение ячейки — через запятую по элементам массива (например `emails - address` = `"a@b.ru, b@b.ru"`).
...
- **0.4.0**
  ...
  - Исключение ключей только внутри пути (`exclude_keys_in_path`): например убрать поле `info` только внутри раздела `full`.
  - Порядок колонок (`column_order`): указанные поля идут первыми на листе.
  - Разбор массивов объектов: значение вида `[{key: val}, ...]` разворачивается в колонки `родитель - ключ`, значения через запятую.
